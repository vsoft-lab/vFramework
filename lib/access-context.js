var assert = require('assert'),
    vframe = require('./vframe'),
    debug = require('debug')('vframe:security:access-context');
...
function AccessContext(context) {
    ...
    model = ('string' === typeof model) ? vframe.getModel(model) : model;
    this.model = model;
    ...
    assert(vframe.AccessToken,
           'AccessToken model must be defined before AccessContext model');
    this.accessToken = context.accessToken || vframe.AccessToken.ANONYMOUS;
...
AccessRequest.prototype.isAllowed = function() {
    return this.permission !== vframe.ACL.DENY;
};
