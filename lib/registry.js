...
var assert = require('assert'),
    ...
    juggler = require('loopback-datasource-juggler'),
    debug = require('debug')('vframe:registry'),
 ...
 * * `vframe.createModel(name, properties, options)`
 * * `vframe.createModel(config)`
 *
 ...
 *
 * @header vframe.configureModel(ModelCtor, config)
 */
 ...
 *
 * @header vframe.findModel(modelName)
 */
...
/**
 * Look up a model class by name from all models created by
 * `vframe.createModel()`. Throw an error when no such model exists.
 *
 * @param {String} modelName The model name
 * @returns {Model} The model class
 *
 * @header vframe.getModel(modelName)
 */
 ...
 *
 * @header vframe.createDataSource(name, options)
 */
 ...
 *
 * @header vframe.setDefaultDataSourceForType(type, dataSource)
 */
 ...
 * @param {String} type The datasource type.
 * @returns {DataSource} The data source instance.
 * @header vframe.getDefaultDataSourceForType(type)
 */
...
// temporary alias to simplify migration of code.
Object.defineProperty(registry, 'DataModel', {
    get: function() {
        var stackLines = new Error().stack.split('\n');
        console.warn('vframe.DataModel is deprecated, ' +
                     'use vframe.PersistedModel instead.');
